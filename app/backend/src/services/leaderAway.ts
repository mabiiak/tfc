...
const incrementTable = (row: IBorder, info: IMatch) => {
  ...
  teamRow.goalsFavor += info.awayTeamGoals;
  teamRow.goalsOwn += info.homeTeamGoals;
  teamRow.goalsBalance += info.awayTeamGoals - info.homeTeamGoals;
...
async function getTable() {
  ...
  const table = allTeams.map((team) => {
    ...
    allMatches
      .filter((matche) => matche.awayTeam === team.id)
...
export default async function orderTableAway() {
